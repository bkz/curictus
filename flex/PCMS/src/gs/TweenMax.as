...
DESCRIPTION:
	...
	you prefer. Currently, TweenMax adds the following features (compared to TweenFilterLite):
		...
		- Kill individual properties of a tween with killProperties()
		- Pause/Resume all tweens and/or delayed calls
...
NOTES:
	...
	- You can tween the volume of any MovieClip (or any item with a soundTransform property) using the tween 
	  property "volume", like: TweenMax.to(myClip_mc, 1.5, {volume:0});
...
package gs {
	...
	public class TweenMax extends TweenFilterLite implements IEventDispatcher {
		public static var version:Number = 2.17;
		...
		public function reverse($adjustDuration:Boolean=true):void {
			...
			if ($adjustDuration && _curTime > this.initTime + (this.delay * 1000)) {
				this.initTime = _curTime - ((1 - this.progress) * this.duration * 1000 / _timeScale) - (this.delay * 1000);
				if (_pauseTime == -1) { //required for OverwriteManager
					this.startTime = this.initTime + (this.delay * 1000);
				}
			}
			resume();
...
//---- STATIC FUNCTIONS -----------------------------------------------------------------------------------------------------------
		...
		public static function delayedCall($delay:Number, $onComplete:Function, $onCompleteParams:Array = null):TweenMax {
			return new TweenMax($onComplete, 0, {delay:$delay, onComplete:$onComplete, onCompleteParams:$onCompleteParams, overwrite:0});
		}
...
//---- GETTERS / SETTERS ----------------------------------------------------------------------------------------------------------
		...
		public function get progress():Number {
			var n:Number = (((_curTime - this.initTime) / 1000) - this.delay) / this.duration * _timeScale || 0;
			if (n > 1) {
				return 1;
			} else {
				return n;
