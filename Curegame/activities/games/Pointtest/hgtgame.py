...
# Are we using some random quick hack to display results in-game?
VITALIS = False
...
class Game(hgt.game.Game):
    def build(self):
        ...
        self.dingSound = Sound("../../../media/sounds/ding.wav", intensity = 0.5)
        self.clickSound = Sound("../../../media/sounds/click.wav", intensity = 0.5)
    ...
    def press_target(self):
        if not self.targetLocked:
            ...
            self.targetMat.emissiveColor = RGB(0.4, 0.4, 0)
    ...
    def display_target(self):
        ...
        self.targetMat.emissiveColor = RGB(0, 0.2, 0)
    ...
    def post_trial(self):
        ...
        if not VITALIS:
            ...
            self.dingSound.play()
            t = TextShape()
            t.material.emissiveColor = RGB(1, 1, 1)
            t.text.string = [
                "Din tid:",
                "%.02f" % (hgt.time.now - self.startTime)
            ]
            self.add_child(t.node)
            #hgt.keyboard.bind_key(key = " ", onPress = self.quit, noEvent = True)
            self.quitButtonToggle.hapticsOn = True
            self.quitButtonToggle.graphicsOn = True
...
<ToggleGroup DEF="TOGGLE" >
 ...
 rotation="1.0 0.0 0.0  1.57">
   <!-- Back side turned off, as ordered 2011-04-01 -->
   ...
   <Shape >
     ...
     <Disk2D DEF="GEOMETRY" outerRadius="0.038" solid="true" />
     <!--
     <Cylinder DEF="GEOMETRY" radius="0.038" height="0.001"
...
""")
    ...
    def build_quitter(self):
        quitButton = self.world.find("QuitButton")
        self.quitButtonToggle = hgn.ToggleGroup(graphicsOn = False, hapticsOn = False)
        quitButton.reparent(self.quitButtonToggle)

        self.quitButtonListener = ForceListener(
            callbackObject = None,
            callback = self.quit_button_press,
            hitForce = TARGET_PRESSURE
        )

        self.quitButtonMesh = self.world.find("ME_QuitButton")
        self.quitButtonMesh.h3dNode.force.routeNoEvent(self.quitButtonListener)
