...
class QuitAPIField: public PeriodicUpdate< SFInt32 > {
	virtual void update() {
		int key = static_cast< SFInt32 * >(routes_in[0])->getValue();
		if (key == KeySensor::F9) {
			if (IDOK == ::MessageBox(HWND_DESKTOP, L"Force crash?", L"VRS H3D", MB_OKCANCEL)) {
				*(char*)(0) = 0;
			}
		}		
		if (key == KeySensor::F10) {
			if (IDOK == ::MessageBox(HWND_DESKTOP, L"Force hang?", L"VRS H3D", MB_OKCANCEL)) {
				for (;;) { 
					::Sleep(1); 
				}
			}
		}
		if (key == KeySensor::F11) {
			if (IDOK == ::MessageBox(HWND_DESKTOP, L"Normal exit?", L"VRS H3D", MB_OKCANCEL)) {
		...
		if (key == KeySensor::F12) {
			if (IDOK == ::MessageBox(HWND_DESKTOP, L"Force exit?", L"VRS H3D", MB_OKCANCEL)) {
...
int h3dmain(int argc, char* argv[]) {
#ifdef H3DAPI_LIB
  initializeH3D();
#endif
...

  {
    ...
    ks->actionKeyPress->route( quit_api );
	...
	SetupCurictusWindowStyle(glwindow);
